...
class RAGPipelineOrchestrator:
    def __init__(self, config: Optional[RAGPipelineConfig] = None):
        ...
        self._populate_with_sample_data()
        ...
        if IForest:
            try:
                ...
                logging.warning(f"Failed to initialize anomaly detector: {e}")
        ...
        if GCNConv:
            try:
                ...
                logging.warning(f"Failed to initialize graph model: {e}")
        ...
        try:
            ...
            if collection_info['document_count'] == 0 or force_rebuild:
                ...
                ingestion_stats = self.ingestion_manager.full_reingest(
                    mitre_csv_path, payload_csv_path, cyberagents_path, datasets_dir
                )
    ...
    def _populate_with_sample_data(self):
        collection_info = self.vector_db.get_collection_info()
        
        if collection_info['document_count'] == 0:
            logging.info("Database is empty - populating with cybersecurity sample data")
            
            sample_data = [
                {
                    'content': 'SQL injection attack techniques include UNION SELECT, time-based blind SQL injection, error-based injection, and boolean-based blind injection. These attacks exploit application inputs to manipulate SQL queries.',
                    'metadata': {'type': 'attack', 'severity': 'high', 'technique': 'sql_injection'},
                    'mitre_id': 'T1505.001'
                },
                {
                    'content': 'Cross-site scripting XSS vulnerabilities occur when applications display user input without proper encoding or validation. Types include stored XSS, reflected XSS, and DOM-based XSS.',
                    'metadata': {'type': 'attack', 'severity': 'medium', 'technique': 'xss'},
                    'mitre_id': 'T1059.007'
                },
                {
                    'content': 'Command injection attacks manipulate application inputs to execute arbitrary commands on the host operating system. Common vectors include system functions and shell metacharacters.',
                    'metadata': {'type': 'attack', 'severity': 'critical', 'technique': 'command_injection'},
                    'mitre_id': 'T1059'
                },
                {
                    'content': 'Authentication bypass techniques include credential manipulation, session hijacking, privilege escalation, and multi-factor authentication bypass methods.',
                    'metadata': {'type': 'attack', 'severity': 'high', 'technique': 'auth_bypass'},
                    'mitre_id': 'T1078'
                },
                {
                    'content': 'Network security threats include port scanning, DDoS attacks, DNS tunneling, lateral movement, and network sniffing activities.',
                    'metadata': {'type': 'attack', 'severity': 'medium', 'technique': 'network_attack'},
                    'mitre_id': 'T1046'
                }
            ]
...
def create_rag_pipeline(mitre_csv_path: str = None, payload_csv_path: str = None, 
    ...
    pipeline._populate_with_sample_data()
    return pipeline
