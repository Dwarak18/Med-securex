...
class AdaptiveConfidenceCalibrator:
    ...
    def update(self, scores: List[float]) -> None:
        for s in scores:
            self.num_samples += 1
            delta = s - self.running_avg
            self.running_avg += delta / self.num_samples
            delta2 = s - self.running_avg
            self.running_var += delta * delta2

    def get_stats(self) -> tuple:
        if self.num_samples <= 1:
            return (self.running_avg, 0.0)
        return (self.running_avg, (self.running_var / (self.num_samples - 1)) ** 0.5)
...
class AdvancedRAGRetriever:
    def __init__(self, vector_db: Optional[CybersecurityVectorDB] = None):
        ...
        if HuggingFacePipeline and hf_pipeline:
            try:
                ...
                logging.warning(f"Failed to initialize LLM in AdvancedRAGRetriever: {e}")
                self.llm = None
        else:
            self.llm = None
    ...
    def retrieve(self, query: str, top_k: int = 10, 
        ...
        self.confidence_calibrator.update([r.relevance_score for r in top_results])
        confidence_score = self.confidence_calibrator.calibrate(confidence_score)
